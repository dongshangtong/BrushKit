...
open class MNDataExporter {
    ...
    public init(canvas: MNCanvas) {
        ...
        content = MNCanvasContent(size: canvas.size,
                                lineStrips: data?.elements.compactMap { $0 as? MNLineStrip } ?? [],
                                chartlets: data?.elements.compactMap { $0 as? MNChartlet } ?? [])
        textures = canvas.textures
    }
    
    private var content: MNCanvasContent
    private var textures: [MNTexture]
    ...
    open func saveSynchronously(to directory: URL, identifier: String? = nil,
        ...
        /// save document info
        var info = MNDocumentInfo(identifier: identifier)
