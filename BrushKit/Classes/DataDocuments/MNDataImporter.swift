...
open class MNDataImporter {
    ...
    public static func importData(from directory: URL, to canvas: MNCanvas, progress: ProgressHandler? = nil, result: ResultHandler? = nil) {
    ...
    public static func importDataSynchronously(from directory: URL, to canvas: MNCanvas, progress: ProgressHandler? = nil) throws {
        
        let decoder = JSONDecoder()
        
        /// check infomations
        let infoData = try Data(contentsOf: directory.appendingPathComponent("info"))
        let info = try decoder.decode(MNDocumentInfo.self, from: infoData)
        ...
        let content = try decoder.decode(MNCanvasContent.self, from: contentData)
        ...
        if let scrollable = canvas as? MNScrollableCanvas, let size = content.size {
