...
public protocol MNCanvasElement: Codable {
    ...
    func drawSelf(on target: MNRenderTarget?)
}

/// clear action, a command to clear the canvas
public struct ClearAction: MNCanvasElement {
...
open class MNCanvasData {
    ...
    open func append(lines: [MNLine], with brush: MNBrush) {
        guard lines.count > 0 else {
            return
        }
        // append lines to current line strip
        if let lineStrip = currentElement as? MNLineStrip, lineStrip.brush === brush {
            lineStrip.append(lines: lines)
        } else {
            finishCurrentElement()
            
            let lineStrip = MNLineStrip(lines: lines, brush: brush)
    ...
    open func append(chartlet: MNChartlet) {
        finishCurrentElement()
    ...
    private(set) var undoArray: [MNCanvasElement] = []
    ...
    // MARK: - Observers
    internal var observers = MNDataObserverPool()
    
    // add an observer to observe data changes, observers are not retained
    open func addObserver(_ observer: MNDataObserver) {
        // pure nil objects
        observers.clean()
    ...
    // MARK: - EventHandler
    public typealias EventHandler = (MNCanvasData) -> ()
