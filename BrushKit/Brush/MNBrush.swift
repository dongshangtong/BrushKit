...
open class MNBrush {
    ...
    /// target to draw
    open private(set) weak var target: MNCanvas?
    ...
    internal var renderingColor: MNColor = MNColor(red: 0, green: 0, blue: 0, alpha: 1)
    
    // called when color or opacity changed
    private func updateRenderingColor() {
        renderingColor = color.toMNColor(opacity: opacity)
    }
    ...
    open func makeLine(from: Pan, to: Pan) -> [MNLine] {
    ...
    open func makeLine(from: CGPoint, to: CGPoint, force: CGFloat? = nil, uniqueColor: Bool = false) -> [MNLine] {
        let force = force ?? forceOnTap
        let scale = scaleWithCanvas ? 1 : canvasScale
        let line = MNLine(begin: (from + canvasOffset) / canvasScale,
    ...
    open func finishLineStrip(at end: Pan) -> [MNLine] {
        return []
    }
    ...
    internal func render(lineStrip: MNLineStrip, on renderTarget: MNRenderTarget? = nil) {
...
extension MNBrush {
    ...
    open func pan(from: Pan, to: Pan) -> MNLine {
        return makeLine(from: from, to: to).first!
    }
    
    @available(*, deprecated, message: "", renamed: "makeLine(from:to:force:)")
    open func line(from: CGPoint, to: CGPoint, force: CGFloat = 1) -> MNLine {
